...
class ID3v1(FieldSet):
    ...
    def createFields(self):
        ...
        yield String(self, "song", 30, "Song title", strip=" \0")
        yield String(self, "author", 30, "Author", strip=" \0")
        yield String(self, "album", 30, "Album title", strip=" \0")
        yield String(self, "year", 4, "Year", strip=" \0")
        ...
        if bytes[1] != ' ':
            if bytes[0] != 0x00:
                # ID3 v1.1b
                yield String(self, "comment", 29, "Comment", strip=" \0")
                yield UInt8(self, "track_nb", "Track number")
            else:
                # ID3 v1.1
                yield String(self, "comment", 30, "Comment", strip=" \0")
            yield Enum(UInt8(self, "genre", "Genre"), self.genre_name)
        else:
            # ID3 v1.0
            yield String(self, "comment", 31, "Comment", strip=" \0")
