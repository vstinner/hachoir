from hachoir.core import field as field_module
...
def convert_field(field, new_type_name):
    ...
    if has_static_size(new_type):
        new_field = new_type(field_set, field._getName(),
                             field._getDescription())
    else:
        new_field = new_type(field_set, field._getName(), convert_size(
            field, new_type), field._getDescription())
